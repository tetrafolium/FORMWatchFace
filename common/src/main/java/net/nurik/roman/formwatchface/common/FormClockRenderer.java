...
public class FormClockRenderer {
  private static final long DEBUG_TIME_MILLIS =
      0; // new Date(2015, 2, 22, 12, 59, 52).getTime();
  ...
  private void initOffsGlyphBitmap() {
    ...
    mOffsGlyphBitmap = Bitmap.createBitmap(mOffsGlyphBitmapUnpaddedSize * 2,
  ...
  public void setPaints(final ClockPaints paints) { mPaints = paints; }

  public void updateTime() {
    mTimeMillis = System.currentTimeMillis();
    if (DEBUG_TIME_MILLIS > 0) {
      long v =
          DEBUG_TIME_MILLIS + (System.currentTimeMillis() - BOOT_TIME_MILLIS);
      mTimeMillis = v;
    }
  ...
  public void updateGlyphsAndAnimDuration(final String currentTimeStr,
    ...
    if (DEBUG_GLYPH != null) {
      ...
      for (int i = 0; i < DEBUG_TIME.length(); i++) {
        mGlyphs[mGlyphCount++] =
            mFont.getGlyph(Character.toString(DEBUG_TIME.charAt(i)));
      }
    } else {
    ...
    for (int i = 0; i < mAnimatedGlyphIndexCount; i++) {
      mAnimatedGlyphIndices[i] =
          mTempAnimatedGlyphIndices[mAnimatedGlyphIndexCount - i - 1];
    }
  ...
  public long timeToNextAnimation() { return mMillisToNext - mAnimDuration; }

  public PointF measure(final boolean allowAnimate) {
    mMeasuredSize.set(0, 0);
    layoutPass(new LayoutPassCallback() {
      @Override
      public void visitGlyph(final Glyph glyph, final float glyphAnimProgress,
                             final RectF rect) {
  ...
  private void layoutPass(final LayoutPassCallback cb,
                          final boolean allowAnimate, final RectF rectF) {
    float x = 0;
    ...
    for (int i = 0; i < mGlyphCount; i++) {
      ...
      float glyphWidth =
          glyph.getWidthAtProgress(t) * mOptions.textSize / Font.DRAWHEIGHT;

      rectF.set(x, 0, x + glyphWidth, mOptions.textSize);
      cb.visitGlyph(glyph, t, rectF);

      x += Math.floor(glyphWidth + (i >= 0 ? mOptions.charSpacing : 0));
    }
  }

  public void draw(final Canvas canvas, final float left, final float top,
                   final boolean allowAnimate, final boolean offscreenGlyphs) {
    ...
    layoutPass(new LayoutPassCallback() {
      ...
      public void visitGlyph(final Glyph glyph, final float glyphAnimProgress,
        ...
        if (offscreenGlyphs) {
          ...
          mOffsGlyphCanvas.translate(mOffsGlyphBitmapUnpaddedSize / 2,
                                     mOffsGlyphBitmapUnpaddedSize / 2);
        ...
        if (offscreenGlyphs) {
          canvas.translate(-mOffsGlyphBitmapUnpaddedSize / 2,
                           -mOffsGlyphBitmapUnpaddedSize / 2);
  ...
  private float getGlyphAnimProgress(final int glyphIndex) {
    ...
    if (mAnimatedGlyphIndexCount > 1) {
      glyphStartAnimTime =
          interpolate(accelerate5(indexIntoAnimatedGlyphs * 1f /
                                  (mAnimatedGlyphIndexCount - 1)),
                      0, mAnimDuration - mOptions.glyphAnimDuration);
    }
    return progress(mAnimTime, glyphStartAnimTime,
                    glyphStartAnimTime + mOptions.glyphAnimDuration);
  }
  ...
  private class Font {
    ...
    private void arcTo(final float l, final float t, final float r,
                       final float b, final float startAngle,
                       final float sweepAngle, final boolean forceMoveTo) {
      tempRectF.set(l, t, r, b);
      path.arcTo(tempRectF, startAngle, sweepAngle, forceMoveTo);
    }

    private void drawArc(final float l, final float t, final float r,
                         final float b, final float startAngle,
                         final float sweepAngle, final boolean useCenter,
                         final Paint paint) {
      tempRectF.set(l, t, r, b);
      canvas.drawArc(tempRectF, startAngle, sweepAngle, useCenter, paint);
    }

    private void drawRoundRect(final float l, final float t, final float r,
                               final float b, final float rx, final float ry,
                               final Paint paint) {
      tempRectF.set(l, t, r, b);
      canvas.drawRoundRect(tempRectF, rx, ry, paint);
    }

    private void drawOval(final float l, final float t, final float r,
                          final float b, final Paint paint) {
      tempRectF.set(l, t, r, b);
    ...
    private void drawArc(final float l, final float t, final float r,
                         ...
                         final float sweepAngle, final boolean useCenter,
                         final int color) {
      drawArc(l, t, r, b, startAngle, sweepAngle, useCenter,
              mPaints.fills[color]);
      if (mPaints.hasStroke) {
        drawArc(l, t, r, b, startAngle, sweepAngle, useCenter,
                mPaints.strokes[color]);
      }
    }
    ...
    private void drawRect(final float l, final float t, final float r,
                          final float b, final int color) {
    ...
    private void initGlyphs() {
      mGlyphMap.put("0_1", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          // 1
          if (d2 > 0) {
            drawRect(interpolate(d2, 28, 0), interpolate(d2, 72, 0), 100,
                     interpolate(d2, 144, 48), COLOR_2);
      ...
      mGlyphMap.put("1_2", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          if (d1 > 0) {
            ...
            drawRect(interpolate(d2, 72, 8), 0, interpolate(d2, 144, 108), 72,
                     COLOR_1);
            canvas.restore();
          ...
          if (d > 0) {
            ...
            drawRect(interpolate(d, 28, 0), interpolate(d, 72, 0), 100,
                     interpolate(d, 144, 48), COLOR_2);
        ...
        public void draw(final float t) {
          ...
          if (d < 1) {
            ...
            if (d == 0) {
              ...
              drawArc(108 - 36, interpolate(d, 0, 72), 108 + 36,
                      72 + interpolate(d, 0, 72), -90, 180, true, COLOR_1);
              drawRect(interpolate(d, 8, 72), interpolate(d, 0, 72),
                       interpolate(d, 108, 144), interpolate(d, 72, 144),
                       COLOR_1);
            }
            drawRect(72, 72, 144, 144, COLOR_2);
            ...
            // bottom rectangle
            drawRect(interpolate(d1, 56, 0), interpolate(d1, 72, 96),
                     interpolate(d1, 128, 80), interpolate(d1, 144, 144),
                     COLOR_1);
            ...
            path.close();
            drawPath(path, COLOR_3);
            drawRect(interpolate(d1, 56, 0), 0, interpolate(d1, 128, 80),
                     interpolate(d1, 72, 48), COLOR_3);
            canvas.restore();

            // middle rectangle
            canvas.save();
            drawRect(interpolate(d1, 56, 32), interpolate(d1, 72, 48),
                     interpolate(d1, 128, 80), interpolate(d1, 144, 96),
                     COLOR_2);
            canvas.restore();
          }
        }
      ...
      mGlyphMap.put("3_4", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          if (d1 > 0) {
            ...
            // middle rectangle
            drawRect(interpolate(d2, 72, 0), interpolate(d2, 144, 72), 144,
                     interpolate(d2, 144, 108), COLOR_1);
            ...
            // top rectangle
            drawRect(72, interpolate(d2, 72, 0), 144, interpolate(d2, 144, 72),
                     COLOR_3);
          }
        }
      ...
      mGlyphMap.put("4_5", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          if (d < 1) {
            // bottom rectangle
            drawRect(interpolate(d, 72, 0), 108, interpolate(d, 144, 72), 144,
                     COLOR_2);
            ...
            // middle rectangle
            drawRect(0, 72, interpolate(d, 144, 72), interpolate(d, 108, 144),
                     COLOR_1);
          } else {
            // 5
            // wing rectangle
            canvas.save();
            drawRect(80, interpolate(d1, 72, 0), interpolate(d1, 80, 128),
                     interpolate(d1, 144, 48), COLOR_2);
            canvas.restore();
            ...
            // middle rectangle
            drawRect(0, interpolate(d1, 72, 0), 80, interpolate(d1, 144, 96),
                     COLOR_1);
          }
        }
      ...
      mGlyphMap.put("5_6", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          if (d1 == 0) {
            ...
            drawArc(0, 0, 144, 144, -90, 180, true, COLOR_3);
          }
      ...
      mGlyphMap.put("6_7", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          if (d < 1) {
            drawArc(0, 0, 144, 144, interpolate(d, 180, -64f), -180, true,
                    COLOR_3);
          }
      ...
      mGlyphMap.put("7_8", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          if (d1 > 0) {
            ...
            drawRect(interpolate(d2, 48, 96), interpolate(d2, 96, 48), 96, 144,
                     COLOR_2);
            canvas.restore();
          }
          ...
          if (d < 1) {
            // 7 rectangle
            drawRect(interpolate(d, 0, 48), interpolate(d, 0, 96),
      ...
      mGlyphMap.put("8_9", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          if (d < 1) {
            ...
            // vanishing arc
            drawArc(0, 0, 144, 144, -180, interpolate(d1, 180, 0), true,
                    COLOR_1);
      ...
      mGlyphMap.put("9_0", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          drawArc(0, 0, 144, 144, 0, interpolate(d, 0, -180), true, COLOR_3);
      ...
      mGlyphMap.put("1_ ", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          drawRect(interpolate(d1, 0, 28), interpolate(d1, 0, 72), 100,
                   interpolate(d1, 48, 144), COLOR_2);
      ...
      mGlyphMap.put("2_ ", new Glyph() {
        ...
        public float getWidthAtProgress(final float t) {
          return interpolate(
              decelerate5(progress(t, 0, 0.5f)), 144,
      ...
      mGlyphMap.put("3_0", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          drawRect(interpolate(d1, interpolate(d2, 32, 80), 0), 96, 80, 144,
                   COLOR_1);

          // middle rectangle
          drawRect(interpolate(d2, 32, 80), 48, 80, 96, COLOR_2);
      ...
      mGlyphMap.put("5_0", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          if (d < 1) {
            // wing rectangle
            canvas.save();
            drawRect(80, interpolate(d, 0, 48), interpolate(d, 128, 80),
                     interpolate(d, 48, 144), COLOR_2);
            canvas.restore();
          ...
          // middle rectangle
          drawRect(interpolate(d1, 0, 80),
                   interpolate(d, 0, interpolate(d1, 48, 0)), 80,
                   interpolate(d, 96, 144), COLOR_1);
      ...
      mGlyphMap.put("2_1", new Glyph() {
        ...
        public void draw(final float t) {
          ...
          if (d1 < 1) {
            ...
            drawRect(interpolate(d, 8, 28), 0, interpolate(d, 108, 100), 72,
                     COLOR_1);
            canvas.restore();
