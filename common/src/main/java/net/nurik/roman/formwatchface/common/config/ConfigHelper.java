...
public class ConfigHelper {
  ...
  public static final String KEY_SHOW_NOTIFICATION_COUNT =
  ...
  private static final Set<String> STRING_CONFIG_KEYS =
      new HashSet<>(Arrays.asList(KEY_THEME));

  private static final Set<String> BOOLEAN_CONFIG_KEYS =
      new HashSet<>(Arrays.asList(KEY_SHOW_NOTIFICATION_COUNT, KEY_SHOW_DATE,
                                  KEY_SHOW_SECONDS));

  private final Context mContext;
  private GoogleApiClient mGoogleApiClient;

  public ConfigHelper(final Context context) { mContext = context; }
  ...
  public boolean connect() {
    ...
    mGoogleApiClient =
        new GoogleApiClient.Builder(mContext).addApi(Wearable.API).build();
  ...
  public String getLocalNodeId() {
    ...
    if (!localNodeResult.getStatus().isSuccess()) {
      Log.e(TAG, "Error getting local node info: " +
                     localNodeResult.getStatus().getStatusMessage());
    }
  ...
  private void putConfigDataMapToDataLayer(final DataMap configDataMap) {
    ...
    // NOTE: Need to use timestamps because there's a separate data item for the
    // companion and the wearable
  ...
  private DataMap readConfigDataMapFromDataLayer() {
    ...
    DataItemBuffer dataItemBuffer =
        Wearable.DataApi.getDataItems(mGoogleApiClient).await();
    if (!dataItemBuffer.getStatus().isSuccess()) {
      Log.e(TAG, "Error getting all data items: " +
                     dataItemBuffer.getStatus().getStatusMessage());
    }
  ...
  public static boolean isConfigPrefKey(final String key) {
    return BOOLEAN_CONFIG_KEYS.contains(key) ||
