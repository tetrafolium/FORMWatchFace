...
public class CompanionWatchFaceConfigActivity extends AppCompatActivity
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    ScrimInsetsFrameLayout scrimInsetsFrameLayout =
        (ScrimInsetsFrameLayout)findViewById(R.id.scrim_insets_frame_layout);
    scrimInsetsFrameLayout.setOnInsetsCallback(
        new ScrimInsetsFrameLayout.OnInsetsCallback() {
          @Override
    ...
    // Set up theme list
    mMainClockContainerView =
        (ViewGroup)((ViewGroup)findViewById(R.id.clock_container))
            .getChildAt(0);
    mMainClockView =
        (FormClockView)mMainClockContainerView.findViewById(R.id.clock);

    mAnimateClockContainerView =
        (ViewGroup)((ViewGroup)findViewById(R.id.clock_container))
            .getChildAt(1);
    mAnimateClockView =
        (FormClockView)mAnimateClockContainerView.findViewById(R.id.clock);

    mAnimateClockContainerView.setVisibility(View.INVISIBLE);

    setupThemeList();
    String themeId = mSharedPreferences.getString(ConfigHelper.KEY_THEME,
                                                  Themes.DEFAULT_THEME.id);
    updateUIToSelectedTheme(themeId, false);

    registerSharedPrefsListener();

    // Set up complications config fragment
    mConfigComplicationsFragment =
        (ConfigComplicationsFragment)getFragmentManager().findFragmentById(
    ...
    if (mConfigComplicationsFragment == null) {
      ...
      getFragmentManager()
          .beginTransaction()
          .add(R.id.config_complications_container,
               mConfigComplicationsFragment)
          .commit();
    }
    ...
    helper.addPage(R.string.title_complications,
                   R.id.config_complications_container);
        ...
        public void onSharedPreferenceChanged(
          ...
          if (ConfigHelper.isConfigPrefKey(key)) {
            ...
            if (ConfigHelper.KEY_THEME.equals(key)) {
              String themeId = mSharedPreferences.getString(
                  ConfigHelper.KEY_THEME, Themes.DEFAULT_THEME.id);
  ...
  private void setupThemeList() {
    ...
    for (final Theme theme : Themes.THEMES) {
      ...
      holder.theme = theme;
      holder.container = inflater.inflate(R.layout.config_theme_item,
                                          mThemeItemContainer, false);
      holder.button = (ImageButton)holder.container.findViewById(R.id.button);

      LayerDrawable bgDrawable = (LayerDrawable)getResources()
                                     .getDrawable(R.drawable.theme_item_bg)
                                     .mutate();

      GradientDrawable gd =
          (GradientDrawable)bgDrawable.findDrawableByLayerId(R.id.color);
  ...
  private void loadMuzei() {
    ...
    LayerDrawable bgDrawable = (LayerDrawable)getResources()
                                   .getDrawable(R.drawable.theme_muzei_item_bg)
                                   .mutate();
    holder.button.setBackground(bgDrawable);
  ...
  private void updateAndPersistTheme(final Theme theme) {
    mSharedPreferences.edit()
        .putString(ConfigHelper.KEY_THEME, theme.id)
        .apply();
  }
  ...
  @Override
  public void onLoadFinished(final Loader<LoadedArtwork> loader,
  ...
  private void updateUIToSelectedTheme(final String themeId,
  ...
  private void updatePreviewView(final Theme theme,
                                 final ViewGroup clockContainerView,
                                 final FormClockView clockView) {
    if (theme == Themes.MUZEI_THEME) {
      if (mMuzeiLoadedArtwork != null) {
        ...
        clockView.setColors(mMuzeiLoadedArtwork.color1,
                            mMuzeiLoadedArtwork.color2, Color.WHITE);
      }
      ...
      clockView.setColors(res.getColor(theme.lightRes),
                          res.getColor(theme.midRes), Color.WHITE);
