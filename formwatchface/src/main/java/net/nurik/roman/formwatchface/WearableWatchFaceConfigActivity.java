...
public class WearableWatchFaceConfigActivity extends Activity {
  ...
  private static float ROUND_FACTOR =
      0.146467f; //(1 - sqrt(2)/2)/2 (from BoxInsetLayout)
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    mGridViewPager.setAdapter(
        new FragmentGridPagerAdapter(getFragmentManager()) {
          @Override
    ...
    mContainerView.setOnApplyWindowInsetsListener(
        new View.OnApplyWindowInsetsListener() {
          @Override
          public WindowInsets onApplyWindowInsets(final View v,
                                                  final WindowInsets insets) {
            mIsRound = insets.isRound();
            mInsetsRect.set(insets.getSystemWindowInsetLeft(),
            ...
            if (mIsRound) {
              mInsetsRect.left = Math.max((int)(v.getWidth() * ROUND_FACTOR),
                                          mInsetsRect.left);
              mInsetsRect.right = Math.max((int)(v.getWidth() * ROUND_FACTOR),
                                           mInsetsRect.right);
              mInsetsRect.top =
                  Math.max((int)(v.getWidth() * ROUND_FACTOR), mInsetsRect.top);
              mInsetsRect.bottom = Math.max((int)(v.getWidth() * ROUND_FACTOR),
                                            mInsetsRect.bottom);
            }
